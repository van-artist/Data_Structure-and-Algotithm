...
{
    if (headNode == nullptr || listSize <= 1)
        return;

    // 找到链表中最大值，确定最大位数
    T maxValue = headNode->element;
    ChainNode<T> *current = headNode->next;
    ...
    {
        std::vector<ChainNode<T> *> buckets[range]; // 创建桶

        // 按当前位的值分配到桶中
        while (current != nullptr)
        {
            int bucketIndex = (current->element / (int)std::pow(range, digit)) % range;
            buckets[bucketIndex].push_back(current);
            current = current->next;
        }

        // 重新链接桶中的元素
        ChainNode<T> *newHead = nullptr;
        ChainNode<T> *tail = nullptr;
        for (int i = 0; i < range; ++i)
        {
            for (ChainNode<T> *node : buckets[i])
            {
                if (newHead == nullptr)
                {
                    newHead = node;
